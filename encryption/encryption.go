...
func EncryptData(cardtype, cvv string, boolVal bool, pieL, pieE int, pieK string) ([]string, error) {
	...
	data := initEncryption(fmt.Sprint(pieL), fmt.Sprint(pieE), fmt.Sprint(pieK))

	ctx, _ := v8go.NewContext(vm)

	ctx.RunScript(data, "")

	_, err := ctx.RunScript(fmt.Sprintf(`let data = ProtectPANandCVV("%s", "%s", %t)`, cc, cvv, boolVal), "")
	if err != nil {
		return nil, err
	}
	val, err := ctx.RunScript("data", "")
	if err != nil {
		return nil, err
	}

	return strings.Split(fmt.Sprint(val), ","), nil
}
